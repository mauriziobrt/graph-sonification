 ...
 */
const createFaustNode = async (audioContext, metapath, modulepath, dspName = "template", voices = 0, sp = false, bufferSize = 512) => {
    ...
    /** @type {FaustDspMeta} */
    const dspMeta = await (await fetch(metapath)).json();

    // Compile the DSP module from WebAssembly binary data
    const dspModule = await WebAssembly.compileStreaming(await fetch(modulepath));
